...
def Item_CF(trn_user_items, val_user_items, K, N):
    ...
    '''
    # sim是双层字典,用来表示相似性矩阵,作为后面协同过滤的基础
    sim={}
    num = {}
    print('构建item-item之间的相似性矩阵...')
    for uid, items in tqdm(trn_user_items):
        for i in items:
            if i not in num:
                num[i]=0
            num[i]+=1
    ...
    print('计算协同过滤矩阵．．．')
    # sim:{item_id1:{item_id2:nums,..}, item_id3:{item_id4:num2,...},...}
    ...
    # 然后对这K个用户交互的商品中除 当前测试用户在训练集中交互过的商品  以外的商品计算最终的相似度分数
    # 最终推荐的候选商品的相似度分数是由多个相似商品对该商品分数的一个累加和
    items_rank = {}
    print('给用户进行推荐．．．')
    # 遍历验证集的所有测试用户,val_user_items为：{user_id1: [item_id1, item_id2,...,item_idn], user_id2...}
    for uid, _ in tqdm(val_user_items.items()):
        items_rank[uid]={}
        # 遍历该用户过去点击过的历史商品,寻找和点击过的商品相似的商品
        for hist_item in trn_user_items[uid]:
            # 该用户点击过的商品可以看作是query(即sim[hist_item].items()),拿该query去找匹配(相似的)其他商品
    ...
    items_rank = {k:sorted(v.items(), key=lambda x:x[1], reverse=True)[:N] for k,v in val_user_items.items()}
    # 去重
...
if __name__=='__main__':
    ...
    rec_items = Item_CF(trn_user_items, val_user_items, K=80, N=10)
